...
def testNetwork(num_hidden_units, decay, retain_input, retain_hidden):
	...
	x = tf.placeholder(tf.float32, [None, numWords])
	y = tf.placeholder(tf.float32, [None, numHashtags])
	...
	### Construct model
	hidden_layer_1_weights = tf.Variable(tf.random_normal([numWords, NUM_HIDDEN_UNITS], stddev=0.01))
	hidden_layer_2_weights = tf.Variable(tf.random_normal([NUM_HIDDEN_UNITS, NUM_HIDDEN_UNITS], stddev=0.01))
	output_layer_weights = tf.Variable(tf.random_normal([NUM_HIDDEN_UNITS, numHashtags], stddev=0.01))
	...
	# train_x = data.train_set.inputs()
	# train_y = data.train_set.targets()
	...
	for epoch in range(EPOCHS):
		...
		train_accuracy = sess.run(accuracy, feed_dict={x: data.train_set.inputs(), y: data.train_set.targets(), p_retain_input: 1, p_retain_hidden: 1})
		train_accuracies.append(train_accuracy)
		validation_accuracy = sess.run(accuracy, feed_dict={x: data.validation_set.inputs(), y: data.validation_set.targets(), p_retain_input: 1, p_retain_hidden: 1})
...
if __name__ == "__main__":
	...
	accuracies = testNetwork(100, 0.01, 0.8, 0.8)
	print(accuracies)
	exit()
