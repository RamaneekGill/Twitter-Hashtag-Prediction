...
def read_data_sets(debug_mode=False):
  ...
  train_inputs = create_matrix(train_inputs[:int(0.25*len(train_inputs))], tweet_vocabulary).astype(np.int32)
  print('finished train_inputs')
  train_targets = create_matrix(train_targets[:int(0.25*len(train_targets))], hashtag_vocabulary).astype(np.int32)
  print('finished train_targets')
  validation_inputs = create_matrix(validation_inputs[:int(0.25*len(validation_inputs))], tweet_vocabulary).astype(np.int32)
  print('finished validation_inputs')
  validation_targets = create_matrix(validation_targets[:int(0.25*len(validation_targets))], hashtag_vocabulary).astype(np.int32)
  print('finished validation_targets')
  test_inputs = create_matrix(test_inputs[:int(0.25*len(test_inputs))], tweet_vocabulary).astype(np.int32)
  print('finished test_inputs')
  test_targets = create_matrix(test_targets[:int(0.25*len(test_targets))], hashtag_vocabulary).astype(np.int32)
